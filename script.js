...
//         const randomText = generateRandomText(markovChain, 50); // max amount of words in text

//         document.getElementById('output').innerText = randomText;
...
//     // only start with capitalized words
//     const capitalizedWords = keys.filter(word => /^[A-Z]/.test(word));

//     // random pick capitalized word to start
...
//         if (!nextWords || nextWords.length === 0) break; // Stop if no next word

//         word = nextWords[Math.floor(Math.random() * nextWords.length)];
//         result.push(word);

//         // stop if word ends with punctuation
...
//         const text = await response.text();
//         const markovChain = generateMarkovChain(text);
//         const randomText = generateRandomText(markovChain, 100);
